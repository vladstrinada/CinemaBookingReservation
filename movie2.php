...
<p>Nazwa filmu:</p>
 ...
 <?php
include('conn.php');
$query=mysql_query("SELECT * FROM reservation.movies where No=2");
{
...
<p>Czas seansu:</p>
  ...
  <select name="Time">
    <option><?php
include('conn.php');
$query=mysql_query("SELECT * FROM reservation.time where id=4");
{
	$row=mysql_fetch_array($query);
	echo "".$row['time']."";
}
?></option>
    <option><?php
include('conn.php');
$query=mysql_query("SELECT * FROM reservation.time where id=5");
{
	$row=mysql_fetch_array($query);
	echo "".$row['time']."";
}
